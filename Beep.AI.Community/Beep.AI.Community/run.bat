...
REM   run.bat --port=5003        - Run on custom port
REM   run.bat --host=0.0.0.0     - Listen on all interfaces
REM   run.bat --reset-db         - Reset database before starting
REM   run.bat --seed-demo        - Seed demo data before starting
REM   run.bat --reset-db --seed-demo - Reset and seed demo data

cd /d "%~dp0"

REM Check for --reset-db flag
echo %* | findstr /i "\-\-reset-db" >nul
if not errorlevel 1 (
    echo [INFO] Resetting database...
    if exist "instance\community.db" del /f "instance\community.db"
    if exist "community.db" del /f "community.db"
    if exist "instance\setup_complete.json" del /f "instance\setup_complete.json"
...
REM Check for --seed-demo flag
echo %* | findstr /i "\-\-seed-demo" >nul
if not errorlevel 1 (
...
python-embedded\python.exe run_community.py %*
